      ...
      <main className="min-h-0 min-w-0 overflow-y-auto">
        {creatingBase ? (
          <div className="mx-auto w-full max-w-3xl px-5 py-7 sm:px-8 sm:py-10">
            ...
            {!providers.length ? <div className="py-6 text-sm"><p className="font-medium">{t('providerRequired')}</p><p className="mt-1 text-muted-foreground">{t('providerRequiredDescription')}</p><Link href={`/app/${encodeURIComponent(slug)}/settings/providers`} className="ui-button-secondary mt-4 inline-flex">{t('openProviderSettings')}</Link></div> : (
              <div className="grid gap-x-5 gap-y-4 py-6 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2 text-xs font-medium text-muted-foreground">
                  ...
                  <ModelPicker
                    ...
                    onConfigure={() => {
                      const returnTo = `/app/${encodeURIComponent(slug)}/knowledge`;
