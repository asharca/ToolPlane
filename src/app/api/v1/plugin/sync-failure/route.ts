...
export const POST = withRequestLogging("/api/v1/plugin/sync-failure", async function POST(req: Request) {
  ...
  await recordEvent({ domain: 'plugin', eventName: 'plugin.sync.failed', workspaceId: scope.workspaceId,
    outcome: 'error', errorCode: body.reason, attributes: { reportedBy: 'client', toolkitId: scope.toolkitId } });
  return json({ ok: true });
});
