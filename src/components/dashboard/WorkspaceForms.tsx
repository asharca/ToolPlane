'use client';
import { Button as BeuiButton, Input as BeuiInput, Select as BeuiSelect } from '@/components/ui/Controls';
  ...
  return (
    <form action={formAction} className="space-y-3" aria-busy={pending} data-workspace-navigation={action === createWorkspaceAction ? '' : undefined} data-unsaved-changes={pending ? 'true' : undefined}>
      <fieldset disabled={pending} className="min-w-0 space-y-3 disabled:opacity-60">
        {children}
        <BeuiButton nativeButton unstyled type="submit" className={danger ? 'ui-button-secondary text-destructive' : 'ui-button-primary'}>
...
export function CreateWorkspaceForm({ intent = '', autoFocus = false }: { intent?: string; autoFocus?: boolean }) {
  ...
  return (
    <WorkspaceActionForm action={createWorkspaceAction} label={t('create')}>
      <BeuiInput type="hidden" name="intent" value={intent} />
      <label htmlFor={id} className="block text-sm font-medium">{t('name')}</label>
      <BeuiInput id={id} name="name" required maxLength={80} autoFocus={autoFocus} placeholder={t('namePlaceholder')} className="ui-input w-full" />
...
export function RenameWorkspaceForm({ slug, name, canManage }: { slug: string; name: string; canManage: boolean }) {
  ...
  return canManage ? (
    <WorkspaceActionForm action={renameWorkspaceAction} label={t('save')}>
      ...
      <label htmlFor={id} className="block text-sm font-medium">{t('name')}</label>
      <BeuiInput id={id} name="name" defaultValue={name} required maxLength={80} className="ui-input w-full" />
    ...
    <details className="max-w-sm text-sm">
      ...
      <div className="mt-2 rounded-lg border border-border p-3">
        <WorkspaceActionForm action={memberActions[kind]} label={t('confirm')} danger>
          <BeuiInput type="hidden" name="workspace" value={slug} />
          <BeuiInput type="hidden" name="memberId" value={memberId ?? ''} />
          <BeuiInput type="hidden" name="invitationId" value={invitationId ?? ''} />
  ...
  return (
    <details className="space-y-3 text-sm" open={status !== 'active' ? true : undefined}>
      ...
      <form action={action} className="space-y-3" aria-busy={pending}>
        ...
        <BeuiInput id={id} name="confirmation" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} required maxLength={80} autoComplete="off" disabled={pending} className="ui-input w-full" />
        <BeuiButton nativeButton unstyled type="submit" disabled={pending || confirmation !== name} className="ui-button-secondary text-destructive disabled:opacity-50">
  ...
  return (
    <details className="space-y-3 text-sm">
      ...
      <WorkspaceActionForm action={transferWorkspaceOwnershipAction} label={t('confirmTransfer')} danger>
        ...
        <BeuiSelect id={`${id}-member`} name="memberId" required defaultValue="" className="ui-input w-full">
        ...
        <BeuiInput id={`${id}-name`} name="confirmation" required maxLength={80} autoComplete="off" className="ui-input w-full" />
      </WorkspaceActionForm>
    </details>
  );
}
...
export function AcceptWorkspaceInvitationForm({ token }: { token: string }) {
  const t = useTranslations('console.workspaces');
  return <WorkspaceActionForm action={acceptWorkspaceInvitationAction} label={t('acceptInvitation')}><BeuiInput type="hidden" name="token" value={token} /></WorkspaceActionForm>;
}
