...
export const POST = withRequestLogging("/api/v1/agents/[agentId]/conversations/[conversationId]/operations", async function POST(req: Request, { params }: { params: Promise<{ agentId: string; conversationId: string }> }) {
  ...
  try {
    ...
    systemLog('error', '[conversation] operation failed', error);
