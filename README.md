...
the runtime files in `/app`, and exits the Node process. Docker or Coolify then
restarts the same container through `restart: unless-stopped`, so the updater
does not depend on container names generated by a platform.
