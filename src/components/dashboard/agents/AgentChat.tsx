          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <section className="border-b border-border px-3 py-3">
              <div className="mb-2 flex items-center justify-between gap-2 px-1">
                <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Channel sessions</div>
              ...
              <ul className="space-y-1">
                ...
                {conversationGroups.external.length === 0 ? (
                  <li className="rounded-md border border-dashed border-border px-3 py-3 text-xs text-muted-foreground">
                    Connected channels will appear here after their first message.
                  </li>
                ) : null}
              </ul>
            </section>
            ...
            <section className="border-b border-border px-3 py-3">
              <div className="mb-2 flex items-center justify-between gap-2 px-1">
                <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Console chats</div>
                <span className="rounded-md bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">
                ...
                {conversationGroups.consoleChats.map((conversation) => (
                  <li key={conversation.id}>
                    ...
                    >
                      ...
                      <div className="mt-0.5 truncate text-[11px] text-muted-foreground">
                        {conversation.lastMessageAt ? `Last message ${conversation.lastMessageAt}` : 'No messages yet'}
                      </div>
                    </Link>
                  </li>
                ))}
            ...
            <section className="px-3 py-3">
              <div className="mb-2 flex items-center justify-between gap-2 px-1">
                <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Connected channels</div>
                <span className="rounded-md bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">
              ...
              <ul className="space-y-1">
                ...
                {channels.length === 0 ? (
                  <li className="rounded-md border border-dashed border-border px-3 py-3 text-xs text-muted-foreground">
                    Add Telegram, Weixin, WeCom, Discord, or DingTalk from Channels.
                  </li>
                ) : null}
              </ul>
            </section>
        ...
        <section className="ui-panel flex min-h-[34rem] min-w-0 flex-col overflow-hidden">
            ...
            <div className="mt-3 grid gap-2 md:grid-cols-2">
              ...
              <div className="rounded-md border border-border bg-muted/15 px-3 py-2">
                <div className="flex items-center gap-2 text-xs font-medium text-foreground">
                  ...
                  {matchedChannel ? 'Matched channel' : 'Delivery path'}
                </div>
                <div className="mt-1 truncate text-xs text-muted-foreground">
                  {matchedChannel
                    ? `${matchedChannel.name} · ${matchedChannel.platformLabel} · ${matchedChannel.status}`
                    : activeSource
                      ? 'No active channel record matches this platform yet'
          ...
          {!ready ? (
            <div className="mx-5 mt-4 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-700 dark:text-amber-300">
              Pick a provider and model on Settings before chatting.
            </div>
          ) : null}
              ...
              <div className="flex min-h-full items-center justify-center">
                <div className="max-w-md rounded-md border border-dashed border-border bg-card px-5 py-6 text-center">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Send a console message here, or let a connected channel create its own session after the first inbound message.
                  </p>
                </div>
              </div>
            ) : (
          ...
          >
            <div className="flex gap-2">
              <textarea
                ...
                placeholder={conversationId ? 'Message this agent' : 'Create or select a chat first'}
