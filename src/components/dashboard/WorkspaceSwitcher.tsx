    ...
    <Popover.Root onOpenChange={(nextOpen) => !nextOpen && setCreating(false)}>
      <Popover.Trigger asChild>
        <button
          ...
          className={`flex items-center gap-2.5 rounded-md px-2 py-2 text-left transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800 ${
            compact ? 'size-9 justify-center px-0' : 'w-full'
          }`}
        >
          ...
          <ChevronsUpDown className={`size-4 shrink-0 text-muted-foreground ${compact ? 'hidden' : ''}`} />
        </button>
      </Popover.Trigger>
      ...
      <Popover.Portal>
        <Popover.Content
          ...
          className={`z-50 overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-lg dark:border-zinc-700 dark:bg-zinc-900 ${
            compact ? 'w-64' : 'w-[var(--radix-popover-trigger-width)]'
          }`}
        >
