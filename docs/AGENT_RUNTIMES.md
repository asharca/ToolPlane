...
| Dedicated branch of `agents/[agentId]/chat/route.ts` | Saved `conversationId`, when present | Reuses that native session and scopes collaboration task access |
| Dedicated sub-agent branch of `run.ts` | Not supplied | One-shot delegation; does not reuse the parent's native conversation |

Platform messages, Hermes runtime-session fields, and these native session keys are not interchangeable. Direct `/chat` now supplies the saved Conversation ID; a request without a saved conversation still has no persistent native-session guarantee.
...
Selected platform sub-agents additionally enable a scoped collaboration MCP server; see [Agent collaboration](./AGENT_COLLABORATION.md) for tasks, allowlists and user authorization. This is not implicit injection of the old host tool set, nor native CLI-internal sub-agents.
