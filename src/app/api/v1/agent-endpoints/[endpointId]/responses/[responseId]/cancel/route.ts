...
import { requestAgentResponseCancellation } from '@/lib/agents/public-api/runs';
...
export const OPTIONS = withRequestLogging("/api/v1/agent-endpoints/[endpointId]/responses/[responseId]/cancel", async function OPTIONS(request: Request, context: RouteContext) {
  const { endpointId } = await context.params;
  return agentApiPreflight(request, endpointId);
});

export const POST = withRequestLogging("/api/v1/agent-endpoints/[endpointId]/responses/[responseId]/cancel", async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const response = await requestAgentResponseCancellation({
