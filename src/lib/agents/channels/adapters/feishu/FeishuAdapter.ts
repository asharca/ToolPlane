...
import { type FileAttachment, type ImageAttachment, MAX_FILE_SIZE_BYTES } from '../../media'
...
export class FeishuAdapter extends ChannelAdapter {
  ...
  protected override async performConnect(signal: AbortSignal): Promise<void> {
    if (!this.appId || !this.appSecret) {
      throw new Error('Complete Feishu QR registration or enter app credentials first.')
    }
  ...
  private notifyDrop(chatId: string): void {
    ...
    this.sendMessage(chatId, 'Message could not be processed. Please try again.', replyOptions()).catch((err) => {
