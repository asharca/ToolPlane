    ...
    <fieldset disabled={pending} className="min-w-0 disabled:opacity-60">
      <div className="grid gap-6 py-5 xl:grid-cols-2">
        <form action={action} className="min-w-0 space-y-4">
          ...
          <BeuiInput type="hidden" name="intent" value="retention" />
        ...
        <form action={action} className="min-w-0 space-y-4 border-t border-border pt-5 xl:border-l xl:border-t-0 xl:pl-6 xl:pt-0">
          ...
          <BeuiInput type="hidden" name="intent" value="capture" />
          <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
            <label className="grid min-w-0 gap-1.5 text-xs font-medium">{t('logsCapture')}
              <BeuiSelect name="field" className="ui-input h-10 w-full">{['workspaceId', 'deploymentId', 'agentId'].map((key) => <option key={key} value={key}>{t(`logFields.${key}`)}</option>)}</BeuiSelect>
            </label>
          ...
          <label className="flex items-start gap-2 text-xs text-amber-700 dark:text-amber-300">
            <BeuiInput type="checkbox" name="includeAgentContent" value="confirmed" />{t('logsAgentContentConsent')}
          </label>
      ...
      {settings.captures.length ? <div className="space-y-3 border-t border-border py-4">
        ...
        <form action={action}><BeuiInput type="hidden" name="intent" value="stop" /><Button type="submit" variant="secondary"><CircleStop className="size-4" aria-hidden="true" />{t('logsStopCapture')}</Button></form>
      </div> : null}
    </fieldset>
