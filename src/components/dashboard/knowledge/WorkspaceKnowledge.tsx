'use client';
import { Button as BeuiButton, Input as BeuiInput, Table as BeuiTable } from '@/components/ui/Controls';
          ...
          <div className="mx-auto w-full max-w-3xl px-5 py-7 sm:px-8 sm:py-10">
            ...
            {!providers.length ? <div className="py-6 text-sm"><p className="font-medium">{t('providerRequired')}</p><p className="mt-1 text-muted-foreground">{t('providerRequiredDescription')}</p><Link href={`/app/${encodeURIComponent(slug)}/providers`} className="ui-button-secondary mt-4 inline-flex">{t('openProviderSettings')}</Link></div> : (
              <div className="grid gap-x-5 gap-y-4 py-6 sm:grid-cols-2">
                <label className="sm:col-span-2 text-xs font-medium text-muted-foreground">{t('name')}<BeuiInput value={name} onChange={(event) => setName(event.target.value)} placeholder={t('namePlaceholder')} className="ui-input mt-1.5 h-10 w-full text-foreground" /></label>
                <div className="sm:col-span-2 text-xs font-medium text-muted-foreground">
                  ...
                  <ModelPicker
                    ...
                    trigger={(
                      <BeuiButton nativeButton unstyled type="button" aria-label={`${t('embeddingModel')}: ${model || t('embeddingModel')}`} className="ui-input mt-1.5 flex h-10 w-full items-center gap-2 px-3 text-left text-sm text-foreground">
                ...
                <label className="text-xs font-medium text-muted-foreground">{t('chunkSize')}<BeuiInput type="number" min={200} max={8000} value={chunkSize} onChange={(event) => setChunkSize(Number(event.target.value))} className="ui-input mt-1.5 h-10 w-full text-foreground" /></label>
                <label className="text-xs font-medium text-muted-foreground">{t('chunkOverlap')}<BeuiInput type="number" min={0} value={chunkOverlap} onChange={(event) => setChunkOverlap(Number(event.target.value))} className="ui-input mt-1.5 h-10 w-full text-foreground" /></label>
                <label className="text-xs font-medium text-muted-foreground">{t('resultsPerSearch')}<BeuiInput type="number" min={1} max={20} value={topK} onChange={(event) => setTopK(Number(event.target.value))} className="ui-input mt-1.5 h-10 w-full text-foreground" /></label>
                <label className="text-xs font-medium text-muted-foreground">{t('similarityThreshold')}<BeuiInput type="number" min={-1} max={1} step={0.05} value={threshold} onChange={(event) => setThreshold(Number(event.target.value))} className="ui-input mt-1.5 h-10 w-full text-foreground" /></label>
              </div>
            )}
            {error ? <p role="alert" className="mb-4 text-sm text-destructive">{error}</p> : null}
            {providers.length ? <div className="flex justify-end gap-2 pt-4"><BeuiButton nativeButton unstyled type="button" onClick={() => selected ? setCreatingBase(false) : undefined} disabled={!selected} className="ui-button-secondary">{t('cancel')}</BeuiButton><BeuiButton nativeButton unstyled type="button" onClick={createBase} disabled={busy || !name.trim() || !providerId || !model.trim()} className="ui-button-primary"><Plus className="size-4" />{busy ? t('creating') : t('createBase')}</BeuiButton></div> : null}
          </div>
        ) : selected ? (
          <div className="flex min-h-full flex-col">
            <header className="shrink-0 px-5 py-4 sm:px-7">
              <div className="flex items-start justify-between gap-4"><div className="min-w-0"><h1 className="truncate text-lg font-semibold">{selected.name}</h1><p className="mt-1 truncate text-xs text-muted-foreground">{selected.providerName ?? t('providerUnavailable')} <span aria-hidden="true">·</span> {selected.embeddingModel} <span aria-hidden="true">·</span> {t('documentCount', { count: selected.documents.length })}</p></div><BeuiButton nativeButton unstyled type="button" onClick={() => void deleteBase()} aria-label={t('deleteBase')} title={t('deleteBase')} className="ui-button-ghost ui-icon-button shrink-0 text-muted-foreground hover:text-destructive"><Trash2 className="size-4" /></BeuiButton></div>
              <nav aria-label={t('viewsLabel')} className="mt-4 flex gap-1 overflow-x-auto">{tabs.map((tab) => { const Icon = tab.icon; return <BeuiButton nativeButton unstyled key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} aria-current={activeTab === tab.id ? 'page' : undefined} className={`flex h-8 shrink-0 items-center gap-1.5 rounded-md px-2.5 text-xs font-medium ${activeTab === tab.id ? 'bg-muted text-foreground' : 'text-muted-foreground hover:bg-muted/60 hover:text-foreground'}`}><Icon className="size-3.5" />{tab.label}</BeuiButton>; })}</nav>
            </header>
