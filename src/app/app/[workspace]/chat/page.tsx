...
import { getConversation, listAgents, listConversations } from '@/lib/agents/queries';
...
export default async function WorkspaceChatPage({
  ...
  const activeAgent = agents.find((agent) => agent.id === requestedAgentId) ?? agents[0];
  if (!activeAgent) return null;
  const conversations = await listConversations(activeAgent.id);
  ...
  const loadedConversation = selectedConversationId
    ...
    : null;
  const conversation = loadedConversation?.agentId === activeAgent.id ? loadedConversation : null;
      ...
      <WorkspaceChat
        ...
        conversations={conversations.map((item) => ({
          ...
          lastMessageAt: item.messages[0]?.createdAt
            ...
            : null,
          source: parseMessagingSessionTitle(item.title),
        }))}
      />
    </>
  );
