...
> 文档按 [Diátaxis](https://diataxis.fr/) 思路分类：**概念与架构**（理解系统）、**使用与集成**（完成具体任务）、**运维与发布**（运行系统）、**设计归档**（历史决策，不代表现状）。
...
| 文档 | 内容 |
|---|---|
| [ARCHITECTURE.md](./ARCHITECTURE.md) ([English](./ARCHITECTURE.en.md)) | **总入口**：技术栈、顶层架构、目录结构、68 个 Prisma 模型、控制台/后台/JSON API 全貌 |
| [HERMES_AGENT_RUNTIME.md](./HERMES_AGENT_RUNTIME.md) ([English](./HERMES_AGENT_RUNTIME.en.md)) | Agent runtime 架构决策：Hermes-first、ToolPlane 拥有控制面、Native runtime 的定位 |
...
| [OBSERVABILITY.md](./OBSERVABILITY.md) | （同上）诊断抓取窗口、日志导出、保留期管理 |
| [WORKSPACES.md](./WORKSPACES.md) | （同上）工作区删除迁移的部署注意事项 |

---

## 文档约定

- **单一事实源**：一个主题只在一个文档展开，其余地方用链接引用，不复制段落。
