...
export function AccountMenu({ userLabel, compact = false, isAdmin = false }: { userLabel: string; compact?: boolean; isAdmin?: boolean }) {
    ...
    <Popover.Root>
      ...
      <Popover.Portal>
        <Popover.Content side="top" align="start" sideOffset={8} collisionPadding={8} className="z-50 w-64 max-w-[calc(100vw-2rem)] rounded-xl border border-border bg-popover p-1 text-popover-foreground shadow-xl">
          ...
          <Popover.Close asChild><Link href={ACCOUNT_SETTINGS_HREF} className="ui-button-ghost w-full justify-start"><Settings className="size-4" />{t('account')}</Link></Popover.Close>
