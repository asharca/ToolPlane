...
2. 一个或多个 Model provider（不在 ToolPlane 选择具体模型）。
3. MCP deployments。
4. Installed Skills。
5. Toolkits。
...
Hermes Agent 使用独立的多选 `AgentModelProvider` 关系。所选 `ModelProvider` 会作为稳定命名的
Hermes keyed `providers` 注入，每个条目包含完整缓存模型清单：
...
- models、base URL 和 provider API key 写入 Agent 专属持久卷

首次配置尚无主模型时，ToolPlane 用第一个有缓存模型的 provider/model 作为启动默认值；此后
Hermes 对 `model` 的修改会被保留。主对话、辅助任务、视觉、压缩、委派和 fallback 使用哪个模型，
均由 Hermes 自身配置管理。删除已选 provider 时，仅清理对应的 ToolPlane provider 条目；如果当前
主模型正引用该条目，才回退到剩余 provider 的启动默认值。

Native Agent 继续使用 `Agent.providerId + Agent.model` 的单 provider / 单 model 绑定。
...
Skills 可以直接编辑并持久化。ToolPlane 命名空间下的 `providers` 和
`mcp_servers.toolplane` 始终以 ToolPlane 为准；Hermes 的模型分工、系统提示词及其他原生配置
始终由 Hermes 管理。
...
- `setup_required`：没有选择可注入 Hermes 的 provider。
