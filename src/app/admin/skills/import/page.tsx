  ...
  return (
    <div className="space-y-4 px-8 py-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/skills" className="text-sm text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100">{t('skills')}</Link>
        <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">{t('importSkillBundle')}</h1>
      </div>
      ...
      <form action={formAction} className="max-w-xl space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-zinc-400">
            {t('importsTheWholeFolder')} <code className="font-mono">SKILL.md</code>{t('referencesScriptsAndOtherTextFilesFrontmatterSuppliesNameDescriptionAndAuthor')}
          </p>
        </div>
        ...
        <div className="rounded-lg border border-zinc-100 bg-zinc-50 p-3 text-xs text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900/50 dark:text-zinc-400">
          <p className="font-semibold text-zinc-700 dark:text-zinc-300">{t('examples')}</p>
          <ul className="mt-1 space-y-0.5 font-mono">
            <li>{t('anthropicsskillsskillspdf')}</li>
            <li>https://github.com/anthropics/skills/tree/main/skills/pdf</li>
            <li>{t('openaiskillsskillscuratedplaywright')}</li>
          </ul>
        </div>
