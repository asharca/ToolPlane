...
export async function startProcess(deploymentId: string, name: string): Promise<void> {
  const s = store();
...
export async function restartProcess(deploymentId: string, name: string): Promise<void> {
  await stopProcess(deploymentId);
  await new Promise((r) => setTimeout(r, 250));
  store().delete(deploymentId);
  await startProcess(deploymentId, name);
}
