...
import { FileText, FileUp, LibraryBig, Plus, RefreshCw, Search, Settings2, Trash2, Users } from 'lucide-react';
...
export function WorkspaceKnowledge({
  ...
  async function saveSettings() {
    ...
    setBusy(true); setError(null);
    try {
      const response = await fetch(`/api/v1/knowledge/${selected.id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name, embeddingModel: model, chunkSize, chunkOverlap, topK, threshold }) });
      const body = await response.json() as { error?: string };
      if (!response.ok) throw new Error(body.error || t('settingsError'));
      setBases((current) => current.map((base) => base.id === selected.id ? { ...base, name, embeddingModel: model, chunkSize, chunkOverlap, topK, threshold } : base));
  ...
  return (
    <div className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)] overflow-hidden bg-background lg:grid-cols-[16rem_minmax(0,1fr)] lg:grid-rows-1">
      <aside className="flex max-h-56 min-h-0 flex-col border-b border-border bg-card lg:max-h-none lg:border-b-0 lg:border-r">
      ...
      <main className="min-h-0 min-w-0 overflow-y-auto">
        {creatingBase ? (
          <div className="mx-auto w-full max-w-3xl px-5 py-7 sm:px-8 sm:py-10">
            <div className="border-b border-border pb-5"><h1 className="text-lg font-semibold">{t('newBase')}</h1><p className="mt-1 text-sm text-muted-foreground">{t('newBaseDescription')}</p></div>
            {!providers.length ? <div className="border-b border-border py-6 text-sm"><p className="font-medium">{t('providerRequired')}</p><p className="mt-1 text-muted-foreground">{t('providerRequiredDescription')}</p><Link href={`/app/${encodeURIComponent(slug)}/settings/providers`} className="ui-button-secondary mt-4 inline-flex">{t('openProviderSettings')}</Link></div> : (
              <div className="grid gap-x-5 gap-y-4 py-6 sm:grid-cols-2">
                ...
                <label className="text-xs font-medium text-muted-foreground">{t('embeddingProvider')}<select value={providerId} onChange={(event) => { setProviderId(event.target.value); setModel(providers.find((provider) => provider.id === event.target.value)?.models[0] ?? ''); }} className="ui-input mt-1.5 h-10 w-full text-foreground">{providers.map((provider) => <option key={provider.id} value={provider.id}>{provider.name}</option>)}</select></label>
                <label className="text-xs font-medium text-muted-foreground">{t('embeddingModel')}<input value={model} onChange={(event) => setModel(event.target.value)} className="ui-input mt-1.5 h-10 w-full text-foreground" /></label>
            ...
            {providers.length ? <div className="flex justify-end gap-2 border-t border-border pt-4"><button type="button" onClick={() => selected ? setCreatingBase(false) : undefined} disabled={!selected} className="ui-button-secondary">{t('cancel')}</button><button type="button" onClick={createBase} disabled={busy || !name.trim() || !providerId || !model.trim()} className="ui-button-primary"><Plus className="size-4" />{busy ? t('creating') : t('createBase')}</button></div> : null}
          </div>
        ) : selected ? (
          <div className="flex min-h-full flex-col">
            ...
            {activeTab === 'documents' ? <div className="p-5 sm:p-7">
              <div className="flex flex-col gap-3 border-b border-border pb-5 sm:flex-row sm:items-end">
